...
# SET vars
source ./sockstore/scripts/jenkins/set-vars.sh
...
helm upgrade --install ${RELEASE_NAME} ./sockstore/deploy/kubernetes/${CHART_NAME} -f overwrite-values.yaml --namespace ${CLUSTER_NAMESPACE} --set $newSlot.enabled=true --set $oldSlot.enabled=true --set productionSlot=$oldSlot --reuse-values
...
helm upgrade --install ${RELEASE_NAME} ./sockstore/deploy/kubernetes/${CHART_NAME} -f overwrite-values.yaml --namespace ${CLUSTER_NAMESPACE} --set $newSlot.enabled=true --set $oldSlot.enabled=true --set productionSlot=$newSlot --reuse-values

# Deleting deployment in old slot
helm upgrade --install ${RELEASE_NAME} ./sockstore/deploy/kubernetes/${CHART_NAME} -f overwrite-values.yaml --namespace ${CLUSTER_NAMESPACE} --set $newSlot.enabled=true --set $oldSlot.enabled=false --set productionSlot=$newSlot --reuse-values
