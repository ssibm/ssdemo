...
helm upgrade --install --debug --dry-run ${RELEASE_NAME} ./sockstore/deploy/kubernetes/${CHART_NAME} -f overwrite-values.yaml --namespace ${CLUSTER_NAMESPACE}

echo -e "Deploying into: ${CLUSTER_NAME}/${CLUSTER_NAMESPACE}."

helm upgrade --install ${RELEASE_NAME} ./sockstore/deploy/kubernetes/${CHART_NAME} -f overwrite-values.yaml --namespace ${CLUSTER_NAMESPACE}
